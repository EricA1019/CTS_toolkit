...
const TooltipTemplate = preload("res://addons/tooltips_pro/scripts/tooltip_template.gd")
...
var mouse_tooltip_stack: Array[TooltipTemplate]
var focus_tooltip_stack: Array[TooltipTemplate]

var has_pinned_tooltip: bool
var last_mouse_entered_tooltip: TooltipTemplate
...
func init_tooltip(tooltip_trigger: TooltipTrigger, screen_pos: Vector2) -> TooltipTemplate:
	...
	var template: PackedScene = tooltip_templates.values()[0]
	...
	var instantiated := template.instantiate()
	var new_tooltip: TooltipTemplate = instantiated as TooltipTemplate
	if new_tooltip == null:
		push_error("TooltipManager: template did not produce a Tooltip. Got " + str(instantiated))
		return null
	...
	var stack: Array[TooltipTemplate]
...
func position_tooltip(tooltip: TooltipTemplate, screen_pos: Vector2 = Vector2.ZERO) -> void:
	var offset: Vector2 = tooltip.trigger.offset
	
	# Position tooltip relative to origin element set on TooltipTrigger
	var origin: TooltipEnums.TooltipOrigin = tooltip.trigger.origin
		
	var mouse_position: Vector2 = Vector2.ZERO
	var new_alignment: TooltipEnums.TooltipAlignment = TooltipEnums.TooltipAlignment.TOP_LEFT
	...
	if tooltip.trigger.overflow_mode != TooltipEnums.OverflowMode.OVERFLOW:
		var bounds_rect: Rect2 = get_viewport().get_visible_rect()
		var tooltip_global_rect: Rect2 = tooltip.get_global_rect()
		...
		var clamped_position: Vector2 = tooltip_global_rect.position
...
func get_alignment_position(tooltip: TooltipTemplate, alignment: TooltipEnums.TooltipAlignment, offset: Vector2) -> Vector2:
...
func set_collapse_stop_size(index: int, stack: Array[TooltipTemplate]) -> void:
...
func force_close_stack():
	is_collapsing_stack = false
	var stack_to_close: Array[TooltipTemplate]
	stack_to_close.assign(mouse_tooltip_stack)
	for tooltip in stack_to_close:
		remove_tooltip(tooltip, false)

func remove_tooltip(tooltip: TooltipTemplate, modulate: bool = true) -> void:
