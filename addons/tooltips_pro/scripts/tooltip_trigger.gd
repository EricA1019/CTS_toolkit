...
var active_tooltip: TooltipTemplate
...
func _on_mouse_entered() -> void:
	...
	if TooltipManager.has_pinned_tooltip:
		var owner = self.get_owner()
		var owner_is_tooltip := owner is TooltipTemplate
		if not owner_is_tooltip:
			print("TooltipTrigger: blocked hover because a tooltip is pinned; owner=", owner)
			return
		# Helpful trace when nested tooltip is allowed
		print("TooltipTrigger: allowing nested hover on pinned tooltip; owner=", owner)
