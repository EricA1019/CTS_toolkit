...
const AUTOLOAD_NAME := "CTS_Skills"
const AUTOLOAD_PATH := "res://addons/cts_skills/Core/skills_registry.gd"
const REQUIRED_DEPENDENCIES := ["CTS_Core", "CTS_Entity"]

func _enable_plugin() -> void:
    if not _validate_dependencies():
        push_error("%s requires dependencies: %s" % [AUTOLOAD_NAME, REQUIRED_DEPENDENCIES])
        assert(false, "Missing dependencies for %s" % AUTOLOAD_NAME)
        return
